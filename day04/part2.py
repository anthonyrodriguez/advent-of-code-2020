testRawInput = """ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
def validPassport(passport):
    ...
    for field in passportFields:
        ...
        passportDict[splitFields[0]] = splitFields[1]

    #BYR
    if 'byr' not in passportDict:
        return False
    else:
    ...
    if 'hcl' not in passportDict:
        ...
        if len(hcl) != 7 or hcl[0] != '#':
            ...
            for char in hcl[-6:]:
                char = ord(char)
                if (char < 47 or char > 57) and (char < 97 or char > 122):
                    return False

    #ECL
    if 'ecl' not in passportDict:
        return False
    else:
