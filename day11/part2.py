...
rawInput = """LLLLLLLLLLLLLLL.LL.L.LLLLLLLLLLLLLLL.LLLL.LLLL.LLLLLLLLL.LLLLLLLLLLLLLLLLLLLLLL.LLLLL.LLLLLLLLL
...
UNOCC = 'L'
OCC = '#'
ROW_LEN = len(inputList)
COL_LEN = len(inputList[0])

dirty = True

def get_num_occ_vis_seats(y, x, seatList):
    numOcc = 0

    dis = 1
    while (y - dis >= 0):
        if (seatList[y-dis][x] != EMPTY): # north
            if (seatList[y-dis][x] == OCC):
                numOcc += 1
            break
        dis += 1

    dis = 1
    while (y - dis >= 0 and x - dis >= 0):
        if (seatList[y-dis][x-dis] != EMPTY): # northeast
            if (seatList[y-dis][x-dis] == OCC):
                numOcc += 1
            break
        dis += 1

    dis = 1
    while (y - dis >= 0 and x + dis < COL_LEN):
        if (seatList[y-dis][x+dis] != EMPTY): # northwest
            if (seatList[y-dis][x+dis] == OCC):
                numOcc += 1
            break
        dis += 1
    ...
    dis = 1
    while (y + dis < ROW_LEN):
        if (seatList[y+dis][x] != EMPTY): # south
            if (seatList[y+dis][x] == OCC):
                numOcc += 1
            break
        dis += 1
    ...
    dis = 1
    while (y + dis < ROW_LEN and x - dis >= 0):
        if (seatList[y+dis][x-dis] != EMPTY): # southeast
            if (seatList[y+dis][x-dis] == OCC):
                numOcc += 1
            break
        dis += 1
    ...
    dis = 1
    while (y + dis < ROW_LEN and x + dis < COL_LEN):
        if (seatList[y+dis][x+dis] != EMPTY): # southwest
            if (seatList[y+dis][x+dis] == OCC):
                numOcc += 1
            break
        dis += 1
    ...
    dis = 1
    while (x - dis >= 0):
        if (seatList[y][x-dis] != EMPTY): # east
            if (seatList[y][x-dis] == OCC):
                numOcc += 1
            break
        dis += 1
    ...
    dis = 1
    while (x + dis < COL_LEN):
        if (seatList[y][x+dis] != EMPTY): # west
            if (seatList[y][x+dis] == OCC):
                numOcc += 1
            break
        dis += 1
...
while(dirty):
    ...
    for y in range(ROW_LEN):
        for x in range(COL_LEN):
            if (inputList[y][x] == EMPTY):
            ...
            elif (inputList[y][x] == UNOCC):                
                if (get_num_occ_vis_seats(y, x, inputList) == 0):
                    dirty = True
                    newList[y] += OCC
                else:
                    newList[y] += UNOCC
                ...
                if (get_num_occ_vis_seats(y, x, inputList) >= 5):
                    dirty = True
                    newList[y] += UNOCC
                else:
                    newList[y] += OCC
