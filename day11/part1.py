...
def get_num_occ_adj_seats(y, x, seatList):
    numOcc = 0
    if (y > 0):
        if (seatList[y-1][x] == OCC): # north
            numOcc += 1
        if (x > 0 and seatList[y-1][x-1] == OCC): # northeast
            numOcc += 1
        if (x < COL_LEN - 1 and seatList[y-1][x+1] == OCC): # northwest
            numOcc += 1
    if (y < ROW_LEN - 1):
        if (seatList[y+1][x] == OCC): # south
            numOcc += 1
        if (x > 0 and seatList[y+1][x-1] == OCC): # southeast
            numOcc += 1
        if (x < COL_LEN - 1 and seatList[y+1][x+1] == OCC): # southwest
            numOcc += 1
    if (x > 0 and seatList[y][x-1] == OCC): # east
        numOcc += 1
    if (x < COL_LEN - 1 and seatList[y][x+1] == OCC): # west
        numOcc += 1
    return numOcc
    ...
    for y in range(ROW_LEN):
        for x in range(COL_LEN):
            if (inputList[y][x] == EMPTY):
                ...
                if (get_num_occ_adj_seats(y, x, inputList) == 0):
                    dirty = True
                    newList[y] += OCC
                else:
                    newList[y] += UNOCC
                ...
                if (get_num_occ_adj_seats(y, x, inputList) >= 4):
                    dirty = True
                    newList[y] += UNOCC
                else:
                    newList[y] += OCC
