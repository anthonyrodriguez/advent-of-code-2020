testRawInput = """light red bags contain 1 bright white bag, 2 muted yellow bags.
...
for line in inputLines:
    if line[-14:] != 'no other bags.':
        ...
        for child in childrenSplit:
            ...
            if childColor not in bagDict:
                bagDict[childColor] = [parentColor]
            elif parentColor not in bagDict[childColor]:
                bagDict[childColor].append(parentColor)

def getAncestors(color):
    if color not in bagDict:
        return [color]
    else:
        ancestorList = []
        for parentColor in bagDict[color]:
            ancestorList.append(parentColor)
            ancestorList.extend(getAncestors(parentColor))
        return ancestorList

ancestorBags = set(getAncestors('shiny gold'))
print(len(ancestorBags))
