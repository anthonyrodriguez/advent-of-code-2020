rawTestInput = """1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc"""

rawInput = """4-6 b: bbbdbtbbbj
1-6 g: ggvggbgggstg
1-4 s: lssss
...
def validPassword(passLine):
    splitPassLine = passLine.split()
    passRange = splitPassLine[0].split('-')
    
    i1 = int(passRange[0])
    i2 = int(passRange[1])
    matchLetter = splitPassLine[1][0]
    password = splitPassLine[2]

    valid = False
    if password[i1-1] == matchLetter:
        valid =  not valid
    if password[i2-1] == matchLetter:
        valid = not valid
